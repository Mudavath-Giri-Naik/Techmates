...
/// Pre-game VS screen for Memory Arena duels.
class MemoryPregameScreen extends StatefulWidget {
  final MemoryNotifier notifier;

  const MemoryPregameScreen({super.key, required this.notifier});

  @override
  State<MemoryPregameScreen> createState() => _MemoryPregameScreenState();
}

class _MemoryPregameScreenState extends State<MemoryPregameScreen>
  ...
  void _onNotify() {
    if (!mounted) return;
    // Duel was cancelled
    if (_n.phase == MemoryPhase.modeSelect && !_countingDown) {
    ...
    if (_n.phase == MemoryPhase.countdown && !_countingDown) {
